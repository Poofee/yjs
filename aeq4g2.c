...
double coora[8],*coefa,*prmt,estif[16],*emass,*edamp,*eload;
int num;
...
// .... the following is the stiffness computation
        for (iv=1; iv<=4; ++iv)
        {
            for (jv=1; jv<=4; ++jv)
            {
                ...
                estif[(iv-1)*(4)+jv-1]=estif[(iv-1)*(4)+jv-1]+stif*weigh;
            }
        }
