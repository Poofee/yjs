...
/* subroutine */
void aeq9g3(coora,coefa,prmt,estif,emass,edamp,eload,num)
double coora[18],*coefa,*prmt,estif[81],*emass,*edamp,*eload;
int num;
   ...
   .... num   ---- element no. */
{
    ...
    double ecurlazx[10],ecurlazy[10];
    double x,y,rx,ry;
    double elump[10];
    static double raz[243],caz[27];
...
l2:
        ...
        shapn(nrefc,ncoor,9,&raz[ig_az],caz,crtr,1,3,3);
...
// .... the following is the stiffness computation
        for (iv=1; iv<=9; ++iv)
        {
            for (jv=1; jv<=9; ++jv)
            {
                ...
                estif[(iv-1)*(9)+jv-1]=estif[(iv-1)*(9)+jv-1]+stif*weigh;
            }
        }
